...
startService(){
	...
	if [ $? -eq "0" ]; then
		...
		nohup $JRE_HOME/bin/java -server -Xms68m -Xmx68m -Xmn48m -Xss256K -jar $JAR_NAME >$LOG_NAME 2>&1 &
...
debugService(){
  ...
  if [ $? -eq "0" ]; then
    ...
    nohup $JRE_HOME/bin/java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005 -server -Xms68m -Xmx68m -Xmn48m -Xss256K -jar $JAR_NAME >$LOG_NAME 2>&1 &
    echo $! > $PID
